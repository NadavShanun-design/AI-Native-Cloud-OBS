...
export class LiveKitVideoConverter {
  ...
  async convertVideoElement(video: HTMLVideoElement): Promise<MediaStream> {
    ...
    // TypeScript doesn't have captureStream in OffscreenCanvas types, but it exists
    const stream = (this.canvas as any).captureStream(this.options.frameRate!);
    ...
    this.onFrameCallback = (frame: VideoFrame) => {
      try {
        ...
        // Note: colorSpace not in TypeScript types, but color conversion handled by canvas
        ...
        // Continue processing indefinitely for continuous streaming
    ...
    // Start processing continuously
    this.processVideoElement(video);

    // Return stream immediately - it will keep updating as frames are processed
  ...
  private async processVideoElement(video: HTMLVideoElement): Promise<void> {
    const processFrame = () => {
      if (video.readyState >= 2 && !video.paused && !video.ended && video.videoWidth > 0) {
        ...
        // Process the frame directly (no need to decode since it's already a VideoFrame)
 ...
 * NOTE: Returns a stream that continues processing - DO NOT call cleanup()
 * or the stream will stop producing frames
 */
export async function convertForLiveKit(
  ...
  try {
    ...
    // DO NOT cleanup - the converter needs to keep running to produce frames
    return stream;
  } catch (error) {

...
export default function CameraGrid() {
  ...
  // Filter to only camera participants (identities starting with 'cam-')
  const cameras = useMemo(() => {
    console.log('🎥 Total participants:', remoteParticipants.length);
    console.log('🎥 All participant identities:', remoteParticipants.map(p => p.identity));

    const filtered = remoteParticipants.filter(p => p.identity.startsWith('cam-'));
    console.log('🎥 Filtered cameras:', filtered.length, filtered.map(p => p.identity));
  ...
  const cameraList = useMemo(() => {
    return cameras.map(participant => {
  ...
  return (
    <div className="bg-white/60 backdrop-blur-xl rounded-2xl border border-slate-200/60 p-6 shadow-xl shadow-slate-200/50">
      ...
      <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-xs font-mono">
        ...
        <div className="text-yellow-700">Total Participants: {remoteParticipants.length}</div>
        <div className="text-yellow-700">Participant IDs: [{remoteParticipants.map(p => `"${p.identity}"`).join(', ')}]</div>
        <div className="text-yellow-700">Filtered Cameras: {cameras.length}</div>
        <div className="text-yellow-700">Camera IDs: [{cameras.map(p => `"${p.identity}"`).join(', ')}]</div>
        <div className="text-yellow-700">Scores in Store: {scores.size}</div>
      </div>
